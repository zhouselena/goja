...
func (vc *vmContext) MemUsage(ctx *MemUsageContext) (memUsage uint64, err error) {
	if vc == nil {
...
func intToValue(i int64) Value {
	if i >= -maxInt && i <= maxInt {
		if i >= intCacheMinValue && i <= intCacheMaxValue {
			return intCache[i-intCacheMinValue]
		}
		return valueInt(i)
	}
	return valueFloat(i)
...
func (s *stash) MemUsage(ctx *MemUsageContext) (memUsage uint64, err error) {
...
func (r resolveThisStack) exec(vm *vm) {
	vm.refStack = append(vm.refStack, &thisRef{v: (*valueStack)(&vm.stack), idx: vm.sb})
	vm.pc++
}
